...
var (
	...
	cacheMallProductProductIdPrefix = "cache:mallProduct:product:id:"
)
...
func (m *defaultProductModel) Delete(ctx context.Context, id int64) error {
	mallProductProductIdKey := fmt.Sprintf("%s%v", cacheMallProductProductIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, mallProductProductIdKey)
	return err
}
...
func (m *defaultProductModel) FindOne(ctx context.Context, id int64) (*Product, error) {
	mallProductProductIdKey := fmt.Sprintf("%s%v", cacheMallProductProductIdPrefix, id)
	var resp Product
	err := m.QueryRowCtx(ctx, &resp, mallProductProductIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultProductModel) Insert(ctx context.Context, data *Product) (sql.Result, error) {
	mallProductProductIdKey := fmt.Sprintf("%s%v", cacheMallProductProductIdPrefix, data.Id)
...
func (m *defaultProductModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheMallProductProductIdPrefix, primary)
}
